"""
Simple roundtrip backtranslation baseline for unsupervised machine translation.

The approach:
1. Given monolingual data in language A and B
2. Use a pretrained model to translate A -> B -> A
3. Train on the synthetic parallel data to reconstruct the original sentences using a cross-entropy loss.
"""
...
class BacktranslationTrainer:
    ...
    Training direction (this implementation):
        - src_mono → translate to tgt → train tgt→src (reconstruct src_mono)
          i.e. src → tgt → src and compare reconstructed src to original src.
    """
    ...
    def train_step(
        self,
        src_sentences: List[str],
        bt_tgt_sentences: List[str],  # Backtranslated source sentences in tgt language
        ...
        Training pattern:
            src → tgt (using BT model, offline) → src (train model)
    ...
    def run_evaluation(
        ...
        chrf_src_to_tgt = CHRF(word_order=2, char_order=6).corpus_score(
            hypotheses=all_src_to_tgt,
            references=[tgt_sentences],
        )
        
        chrf_tgt_to_src = CHRF(word_order=2, char_order=6).corpus_score(
            hypotheses=all_tgt_to_src,
            references=[src_sentences],
        )
        
        bleu_src_to_tgt = BLEU(tokenize="flores200").corpus_score(
            hypotheses=all_src_to_tgt,
            references=[tgt_sentences],
        )
        
        bleu_tgt_to_src = BLEU(tokenize="flores200").corpus_score(
            hypotheses=all_tgt_to_src,
            references=[src_sentences],
        )
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            # We pair: (src, bt_tgt) for tgt→src training (src → tgt → src)
            ...
            # Iterate through loader: src → tgt (BT) → src (train)
            for src_batch, bt_tgt_batch in src_loader:
                losses = self.train_step(src_batch, bt_tgt_batch, optimizer)
                ...
                if global_step % log_interval == 0:
                    print(
                        ...
                        f"L_tgt→src (src→tgt→src): {losses['loss_tgt_to_src']:.4f}"
                    )
            ...
            print(f"  Avg Loss (src→tgt→src, tgt→src step): {avg_loss_tgt_to_src:.4f}")
