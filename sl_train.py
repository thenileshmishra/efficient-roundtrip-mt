...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_name",
        type=str,
        default="breakend/nllb-multi-domain",
    ...
    parser.add_argument(
        "--eval_only",
        action="store_true",
        help="Whether to run evaluation only without training.",
    )
...
def main():
    ...
    if args.model_name_or_path:
        tokenizer = NllbTokenizer.from_pretrained(args.model_name_or_path, src_lang=f"sentence_{args.source_lang}", tgt_lang=f"sentence_{args.target_lang}")
    else:
        raise ValueError(
    ...
    def preprocess_function(examples):
        inputs = [ex for ex in examples[f"sentence_{source_lang}"]]
        targets = [ex for ex in examples[f"sentence_{target_lang}"]]
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.with_tracking:
            accelerator.log(
                {
                    "eval/chrf++": eval_metric_chrf["score"],
                    "train/train_loss": total_loss.item() / len(train_dataloader),
                    "train/epoch": epoch,
