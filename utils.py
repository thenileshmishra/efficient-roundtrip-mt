...
def generate_and_return_termination_logprob(
    ...
    active_seqs = torch.ones(encoded_prompt["input_ids"].size(0)).bool().to(encoded_prompt["input_ids"].device)
    state = encoded_prompt["input_ids"].clone()
    log_pf = []
    log_pterm = []
    token_ids = state  # For caching hidden states during generation
    generated_ids = torch.full((encoded_prompt["input_ids"].size(0), 2), termination_token_id, dtype=torch.long, device=model.device)
    ...
    for i in range(max_len + 1):
        ...
        output = model(
                input_ids=encoded_prompt["input_ids"] if encoder_outputs is None else None,
        ...
        generated_ids = torch.cat([generated_ids, token_ids], dim=-1)
    ...
    if skip_rewards:
        ...
        log_r, log_r_unpenalized = reward_fn(generated_ids[:, :-1])
    # add a termination token to the end of the sequence
    return generated_ids, log_pf, log_pterm, log_r, log_r_unpenalized
